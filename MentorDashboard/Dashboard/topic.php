<?php
session_start();

/* Include the db connection file */
include("dbconn.php");

// Function to display profile information
function displayProfile($username, $column)
{
    $dbconn = mysqli_connect("localhost", "root", "", "vbuddy");
    if (!$dbconn) {
        die("Connection failed: " . mysqli_connect_error());
    }

    $sql =  "SELECT b.ASSIGNID, d.USERNAME FROM course a JOIN assign b JOIN mentee c JOIN user d WHERE d.USERNAME='$username' AND b.MENTORID = d.USERID GROUP BY b.ASSIGNID";
    $result = mysqli_query($dbconn, $sql);

    if ($result) {
        if (mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_assoc($result);
            if (isset($row[$column])) {
                return $row[$column];
            } else {
                return "No data found for column: $column";
            }
        } else {
            return "No profile found for username: $username";
        }
    } else {
        return "Error retrieving data: " . mysqli_error($dbconn);
    }

    mysqli_close($dbconn);
}


// if user is logged in successfully, all data from username will be displayed in indexmenteedash.php
if (isset($_SESSION['username'])) {
    $username = $_SESSION['username'];
	
	 // Query the database to retrieve user data
    $sql = "SELECT userID, username, name FROM user WHERE username = '$username'";
    $result = mysqli_query($dbconn, $sql);

    // If there is data in the result, then the code runs
    if ($result && mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        $userID = $row['userID'];
        // Rest of the code
		
}
} else {
    // Redirect to the login page if the user is not logged in
    header("Location: ../Vbuddy/login.html");
    exit();
}
?>

<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8" />
  <meta http-equiv="X-UA-Compatible" content="IE=edge" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Mentor Dashboard</title>
  <link rel="shortcut icon" href="images/university.png" />
  <link href="https://fonts.googleapis.com/icon?family=Material+Icons+Sharp" rel="stylesheet" />
  <link rel="stylesheet" href="css/style.css" />
  <script src="https://kit.fontawesome.com/2b25f4f529.js" crossorigin="anonymous"></script>
  <link rel="stylesheet" href="css3/style.css">

    <style>
        .exam.timetable2 {
            margin-top: 20px; /* Adjust the value as needed */
        }
        header {
            position: relative;
        }

        .exam {
            display: flex;
            align-items: center;
            justify-content: center;
            flex-direction: column;
            height: auto;
            width: 80%;
            margin: auto;
        }
		
		.center {
		  margin: auto;
		  width: 100%;
		  padding: 10px;
		}
		
		.subjects {
		align-items: center;
		width: 160%; /* Set the desired width for the subjects box */
		margin: 35px; /* Center the subjects box horizontally within the container */
	   }
    </style>
</head>

<!-- Mentor Navigation Button -->
<body>
   	<header>
    <div class="logo" title="Student Dashboard">
	
      <a rel="noopener" href="index.html">
        <img src="images/university.png" alt="" />
      </a>
	  
      <a href="indexmentodash.php">
        <h2>Mentor Dashboard</h2>
      </a>
    </div>
	
    <div class="navbar">
      <a href="indexmentodash.php" >
        <i class="fa-solid fa-house fa-lg"></i>
        <h3 style= font-size:10px>Home</h3>
      </a>
	  
      <a href="topic.php"class="active" >
        <i class="fa-solid fa-paperclip fa-lg"></i>
        <h3 style= font-size:10px>Manage Group</h3>
      </a>
	  
      <a href="discussion.php" onclick="timeTableAll()">
        <i class="fa-regular fa-clipboard fa-lg"></i>
        <h3 style= font-size:10px>Discussion</h3>
      </a>
	  
      <a href="report.php" onclick="timeTableAll()">
        <i class="fa-regular fa-clipboard fa-lg"></i>
        <h3 style= font-size:10px>Report & Feedback</h3>
      </a>
      
	  <a href="password.php">
        <span class="material-icons-sharp">password</span>
        <h3  style= font-size:10px>Change Password</h3>
      </a>
	  
      <a href="logout.php">
        <i class="fa-solid fa-right-from-bracket fa-lg"></i>
        <h3 style= font-size:10px>Logout</h3>
      </a>
    </div>
    <div id="profile-btn">
      <i class="fa-light fa-user fa-lg"></i>
    </div>
    <div class="theme-toggler">
      <span class="material-icons-sharp active">light_mode</span>
      <span class="material-icons-sharp">dark_mode</span>
    </div>
  </header>

    <main>
	
	  <div class="subjects">
	         <!---First Textbox -->
			<div class="eg" style="background-color: #dfefc6;">
			  <h3 style= font-size:20px >Total Class/Activities</h3>
			    <small class="text-muted">This is total of class that has been created by you. </small>
				
			
				<?php
				// Assuming you have established a database connection

				// Execute the query to retrieve the assignid for the mentor
				$query = "SELECT count(*) as total_class FROM topic t join assign a ON t.assignid = a.assignid WHERE a.mentorid = '$userID'";
				$result = mysqli_query($dbconn, $query);

				// Check if any assignid is found
				if ($result && mysqli_num_rows($result) > 0) {
					$row = mysqli_fetch_assoc($result);
					$totalclass = $row['total_class'];

					// Display the assignid
					echo "<h3 style='font-size: 45px;'>$totalclass</h3>";
				} else {
					echo "<p>No assignid found for the mentor.</p>";
				}
				?>


			  <small class="text-muted">Group that has been assigned</small>
            </div>
			
	        <!---Second Textbox -->
			<div class="eg" style="background-color: #dfefc6;">
			  <h3 style= font-size:20px >Latest Class ID</h3>
			    <small class="text-muted">Total of mentee that has been assign for your group </small>
				
				<?php
				// Assuming you have established a database connection

				// Execute the query to calculate the total course count
				$query = "SELECT t.topicid FROM topic t JOIN assign a ON t.assignid = a.assignid WHERE a.mentorid = '$userID' ORDER BY DATE DESC LIMIT 1";
				$result = mysqli_query($dbconn, $query);
				$row = mysqli_fetch_assoc($result);
				$topicid = $row['topicid'];

				// Inject the total course count into the HTML template
				  echo "<h3 style= font-size:45px; text-align: center>$topicid</h3>";
				  ?>

			  <small class="text-muted">Limit only 3 mentee for each group</small>
            </div>
			
			<!---Third Textbox -->
			<div class="eg" style="background-color: #dfefc6;">
			  <h3 style= font-size:20px >Total Attendance</h3>
			    <small class="text-muted">Total of attendance that has been created from you to your mentee</small>
				
				<?php
				// Assuming you have established a database connection

				// Execute the query to calculate the total course count
				$query = "SELECT COUNT(*) AS total_attendance FROM attendance a
                          INNER JOIN assign u ON u.mentorid = '$userID'
                          WHERE u.assignid	= a.assignid";
				$result = mysqli_query($dbconn, $query);
				$row = mysqli_fetch_assoc($result);
				$totalAttendance = $row['total_attendance'];

				// Inject the total course count into the HTML template
				  echo "<h3 style= font-size:45px; text-align: center>$totalAttendance</h3>";
				  ?>

			  <small class="text-muted">Last 24 hours ago</small>
            </div>
		  </div>

        <!-- create topic -->
        <div class="exam timetable2">
            <h2>Manage Class/Activities/Topic</h2>
            <table>
                <thead>
                    <form method="POST" action="manage0.php" enctype="multipart/form-data">

					<tr>
					 <th>Class ID</th>
					   <th>
						<div class="center">
						<input type="text" placeholder="Example:- TG0000" style="height: 30px; width: 300px;" name="topic_id" id="topic_id" />
						</div>
					   </th>
					</tr>
					
					<tr>
					  <th>Assigned Group ID:</th>
						<td><div style="font-size: 130%;left: 40%;">
						 <?php echo displayProfile($username, 'ASSIGNID'); ?>
					    </div>
					   </td>
					</tr>
					
					<tr>
					  <th>Date: </th>
						<td>
						<div class="center">
						<input type="date" style="height: 30px; width: 150px;" name="date" id="date" />
						</div>
					   </td>
					</tr>
					
					<tr>
					  <th>Topic Name:</th>
						<td>
						<div class="center">
						<input type="text" style="height: 30px; width: 300px;" name="topic_name" id="topic_name" />
						</div>
					    </td>  
					</tr>
					
					<tr>
					   <th>Attachment(*pdf file only):</th>
						<td>
						
							 <input type="file" name="pdf_file"
							class="form-control" accept=".pdf" required/>
						
					    </td>
					</tr>
					
					<tr>
					  <th>Topic Description:</th>
						<td>
						<div class="center">
						<input type="text" style="height: 30px; width: 300px;" name="description" id="description" />
						</div>
					    </td>
					</tr>
                        <tr>
                      <th>Platform: </th>
                        <td>
                        <div class="center">
                        <input type="text" style="height: 30px; width: 300px;" name="platform" id="platform" />
                        </div>
                        </td>
                    </tr>
                    
                        <tr>
                      <th>Link:</th>
                        <td>
                        <div class="center">
                        <input type="text" style="height: 30px; width: 300px;" name="link" id="link" />
                        </div>
                        </td>
                    </tr>
                    
					
						<tr>
						<td>
					   <div class="form-group">
					<input  style="height: 30px; width: 100px;" type="submit"
						  class="btnRegister" name="submit" value="Create Class">
				</div>
						</td>
						</tr>

			</form>
		</thead>
	</table>
</div>
<p> <center> *Note: Please enter the Topic ID start from "T" in the front, then put your Group ID and enter one digit at the back Example: "TG0001"..</center></p>

<!-- Create Attendance -->
<div class="exam timetable2">
  <h2>Create Attendance</h2>
  <table>
    <thead>
      <form method="POST" action="manage2.php">
        <tr>
          <th>Assigned Group ID:</th>
				<td><div style="font-size: 130%;left: 40%;">
				 <?php echo displayProfile($username, 'ASSIGNID'); ?>
			    </div>
		  </td>
          </th>
		  </tr>
          <tr>
		  
 <th>TopicID:</th>
<th>
    <div class="center">
        <select name="topicid" id="topicid" style="height: 30px; width: 300px;">
            <?php
			// Establish database connection
			$conn = new mysqli('localhost', 'root', '', 'vbuddy');
			if ($conn->connect_error) {
				die("Connection failed: " . $conn->connect_error);
			}

			// Retrieve topics created by the session user for the assigned mentee
			$assignid = displayProfile($username, 'ASSIGNID'); // Assuming you have the assignid available

			$query = "SELECT t.topicid, t.topic
					  FROM topic t
					  INNER JOIN assign a ON t.assignid = a.assignid
					  LEFT JOIN attendance att ON t.topicid = att.topicid AND a.assignid = att.assignid
					  WHERE a.assignid = '$assignid' AND a.mentorid = '$userID'
					  AND att.topicid IS NULL"; // Filter out topics that have already been created in ATTENDANCE table
			$result = $conn->query($query);

			if ($result->num_rows > 0) {
				while ($row = $result->fetch_assoc()) {
					$topicid = $row['topicid'];
					$topic = $row['topic'];
					echo "<option value='$topicid'>$topicid - $topic</option>";
				}
			} else {
				echo "<option value=''>No topics found</option>";
			}

			// Close the database connection
			$conn->close();
			?>

        </select>
    </div>
</th>

		<tr>
		    <th>Date: </th>
				<td>
				   <div class="center">
				   <input type="date" style="height: 30px; width: 150px;" name="date" id="date" />
				   </div>
				</td>
	    </tr>
		
		<tr>
		<th>Start Time: </th>
				<td>
				   <div class="center">
				   <input type="time" style="height: 30px; width: 150px;" name="timestart" id="timestart" />
				   </div>
				</td>
        </tr>
		
		<tr>
		<th>End Time: </th>
				<td>
				   <div class="center">
				   <input type="time" style="height: 30px; width: 150px;" name="timeend" id="timeend" />
				   </div>
				</td>
		
          <td>
            <input style="height: 30px; width: 120px;" type="submit" name="submit" value="Create Attendance">
          </td>
        </tr>
      </form>
    </thead>
  </table>
</div>

 </br>
 </br>
        <?php
        // Close the database connection
        mysqli_close($dbconn);
        ?>
    </main>
    <script src="app.js"></script>
	<footer>
        <div class="footer-container">
            <p><center>Copyright &copy 2023 All rights reserved | QuadraTech </center></p>

        </div>
    </footer>
</body>

</html>