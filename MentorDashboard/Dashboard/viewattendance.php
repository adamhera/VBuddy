...
<header>
  ...
  <div class="exam timetable2">
		...
		if (isset($_GET['topicid'])) {
		  ...
		  $query = "SELECT a.assignid, a.menteeid, u.name, a.dateattend, a.attendstatus
					FROM ATTENDANCE a
		  ...
		  if (mysqli_num_rows($result) > 0) {
			...
			echo "<tr><td><strong>Mentee ID:</strong></td><td>" . $row['menteeid'] . "</td></tr>";
			echo "<tr><td><strong>Name:</strong></td><td>" . $row['name']  . "</td></tr>";
			echo "<tr><td><strong>Date:</strong></td><td>" . $row['dateattend'] . "</td></tr>";
			echo "<tr><td><strong>Status Attendance:</strong></td><td>" . $row['attendstatus']  . "</td></tr>";
            echo "<tr style='height: 55px;'></tr>"; // Add a blank row for spacing
			}

			echo "</table>";
		  } else {
			echo "No attendance found for the given topic ID.";
		  }
		} else {
