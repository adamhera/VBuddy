...
$mentorUsername = $_GET['username']; // Assuming the mentor's username is obtained from the PHP variable

$sql = "SELECT t.TOPICID, DATEATTEND, b.MENTORID, b.MENTEEID,
...
FROM (SELECT DISTINCT TOPICID, ASSIGNID FROM ATTENDANCE) t
LEFT JOIN (
...
LEFT JOIN (
    ...
    FROM ATTENDANCE
    JOIN ASSIGN B
    ON ATTENDANCE.ASSIGNID = B.ASSIGNID
    GROUP BY ATTENDANCE.ASSIGNID, TOPICID, DATEATTEND
) b ON t.TOPICID = b.TOPICID
JOIN ASSIGN ASGN
ON t.ASSIGNID = ASGN.ASSIGNID
JOIN USER U
ON ASGN.MENTORID = U.USERID
WHERE U.USERNAME = '$mentorUsername';";
